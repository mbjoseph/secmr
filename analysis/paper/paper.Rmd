...
Here, we show how visual encounter data can improve abundance estimates in capture-recapture studies for study designs where 1) individual identification requires capture (so that a subset of encountered individuals are captured), and 2) individuals can be encountered at most once per sampling occasion. 
...
However, in the encounter-capture-recapture scenario considered here, we assume that whether an animal is marked or not is unknown until it is captured, as would be the case for subdermal passive integrated transponder tags in an amphibian [@gibbons2004pit]. 
